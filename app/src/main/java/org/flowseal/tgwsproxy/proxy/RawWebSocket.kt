...
class RawWebSocket private constructor(
    ...
    companion object {
        ...
        fun connect(ip: String, domain: String, path: String = "/apiws", timeoutMs: Int = 10000): RawWebSocket {
            ...
            if (statusCode == 101) {
                // Set timeout for ping keepalive (e.g. 20 seconds)
    ...
    private class IdleTimeoutException : Exception()
    ...
    fun recv(): ByteArray? {
        while (!closed) {
            val (opcode, payload) = try {
                readFrame()
            } catch (e: IdleTimeoutException) {
                try {
                    val ping = buildFrame(OP_PING, ByteArray(0), mask = true)
                    synchronized(output) {
                        output.write(ping)
                        output.flush()
                    }
    ...
    private fun readFrame(): Pair<Int, ByteArray> {
        ...
        val b1 = input.read()
        if (b1 == -1) throw java.io.EOFException("Connection closed")
        hdr[1] = b1.toByte()
