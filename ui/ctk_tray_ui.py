...
def _get_doc_url(doc_name: str) -> str:
    ...
    return "https://github.com/Gegaremant/TG-Proxy/blob/main/README.md"
...
def install_tray_config_form(
    ...
    *,
    ...
    ctk.CTkButton(
        header, text="GitHub Repo", width=90, height=28,
        font=(theme.ui_font_family, 12, "bold"), corner_radius=8,
        fg_color="#333333", hover_color="#555555",
        text_color="#ffffff", border_width=0,
        command=lambda: webbrowser.open("https://github.com/Gegaremant/TG-Proxy"),
    ...
    fake_tls_row = ctk.CTkFrame(conn, fg_color="transparent")
    fake_tls_row.pack(fill="x")
    fake_tls_col, fake_tls_var = _labeled_entry(
        ctk, fake_tls_row, theme, "Fake TLS Domain (opt)",
        cfg.get("fake_tls_domain", default_config.get("fake_tls_domain", "")),
        tip="Fake TLS Domain", width=160, pack_fill=True,
    )
    ...
    def _auto_config():
        ...
        def w():
            ...
            btime = 999
            import random
            for sni in random.sample(DEFAULT_SNI_LIST, min(10, len(DEFAULT_SNI_LIST))):
                try:
                    s = time.time()
                    so = socket.create_connection((sni, 443), timeout=1)
                    so.close()
                    t = time.time()-s
                    if t < btime: btime = t; best = sni
                except Exception: pass
            fake_tls_var.set(best)
            try: mb.showinfo("Auto Config", f"Done! Best SNI: {best}")
            except Exception: pass
    ...
    return TrayConfigFormWidgets(
        host_var=host_var, port_var=port_var, secret_var=secret_var, fake_tls_var=fake_tls_var,
...
def validate_config_form(
    ...
    *,
    ...
    fake_tls_val = widgets.fake_tls_var.get().strip()
